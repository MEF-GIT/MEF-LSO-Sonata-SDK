...
## **MEF W115, v0.3**<!-- omit in toc -->

# **LSO Cantata and LSO Sonata Quote Management API - Developer Guide** <!-- omit in toc -->
...
- **[RXXX]** if `instantSyncQuote` equals `false` the
  ...
  **_Note_**: [OXXX] The Seller **MAY** decide to make the
...
**[RXXX]** `product.id` is mandatory for the `quoteItem` in the `delete`
request. [MEF80 R30]
...
**[OXXX]** The Seller **MAY** support the ability for the Buyer to Register for
...
**[RXXX]** If `instantSyncQuote` equals `false` the `relatedContactInformation`
...
**[RXXX]** Each item in `quoteItem` list **MUST** correspond to one and only
one Quote Item in the Buyer's Create Quote request [MEF80 R36], [MEF80 R45].

**[RXXX]** Seller's response **MUST** contain an entry that represents _Seller
...
_Repeated:_ **[RXXX]** If `instantSyncQuote` equals `false` the
...
_Repeated:_ **[RXXX]** If `instantSyncQuote` equals `false` and the Quote Item
...
**[RXXX]** The Product Identifier from relationships having the same
...
**[RXXX]** `MEFGeographicPoint` **MUST** be used to specify Geographic Point.
...
<table id="T_EventSubscriptionInput">
    ...
    <tbody>
        <tr>
          ...
          `http://buyer.co/listenerEndpoint/mefApi/sonata/quoteNotification/v7/listener/quoteStateChangeEvent`</td>
