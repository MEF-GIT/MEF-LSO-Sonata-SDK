...
- [6. API Interactions and Flows](#6-api-interactions-and-flows)
  ...
  - [6.6. Use Case 6: Retrieve Printable Invoice](#66-use-case-6-retrieve-printable-invoice)
- [7. API Details](#7-api-details)
  ...
  - [7.2. Management API Data model](#72-management-api-data-model)
    ...
    - [7.2.2. Common](#722-common)
      ...
      - [7.2.2.5. Type FieldedAddressRepresentation](#7225-type-fieldedaddressrepresentation)
      - [7.2.2.6. Type SubUnit](#7226-type-subunit)
      - [7.2.2.7. Type Money](#7227-type-money)
      - [7.2.2.8. `enum` PaymentMethod](#7228-enum-paymentmethod)
      - [7.2.2.9. Type ProductRef](#7229-type-productref)
      - [7.2.2.10. Type RelatedContactInformation](#72210-type-relatedcontactinformation)
      - [7.2.2.11. Type TaxItem](#72211-type-taxitem)
      - [7.2.2.12. Type TimePeriod](#72212-type-timeperiod)
  ...
  - [7.3. Notification API Data Model](#73-notification-api-data-model)
    ...
    - [7.3.2. Type CustomerBillCreateEvent](#732-type-customerbillcreateevent)
    - [7.3.3. Type CustomerBillCreateEventPayload](#733-type-customerbillcreateeventpayload)
    - [7.3.4. Type CustomerBillStateChangeEvent](#734-type-customerbillstatechangeevent)
    - [7.3.5. Type CustomerBillStateChangeEventPayload](#735-type-customerbillstatechangeeventpayload)
...
for which requirements and use cases are defined in MEF 134 _Billing and Invoice
Business Requirements and Use Cases_ [[MEF 134](#8-references)]. This standard
consists of this document and complementary API definitions for:
...
- [`productApi/billing/billingManagement.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Sonata-SDK/aaa03d484f98664a5a14f4f54f47b675d7efb3b8/productApi/billing/billingManagement.api.yaml)
- [`productApi/billing/billingNotification.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Sonata-SDK/aaa03d484f98664a5a14f4f54f47b675d7efb3b8/productApi/billing/billingNotification.api.yaml)
...
- [`productApi/billing/billingManagement.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Cantata-SDK/83d6edd0c70386058a9af6e677c069b498671da7/productApi/billing/billingManagement.api.yaml)
- [`productApi/billing/billingNotification.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Cantata-SDK/83d6edd0c70386058a9af6e677c069b498671da7/productApi/billing/billingNotification.api.yaml)
...
<tr>
  ...
  <td><a href="#8-references">[MEF 134]</td>
</tr>
<tr>
  <td>Buyer</td>
...
<tr>
  ...
  <td><a href="#8-references">[MEF 134]</a></td>
</tr>
<tr>
...
as defined in MEF 134 _Billing and Invoice Business Requirements and Use Cases_
[[MEF 134](#8-references)].
...
appears in [[MEF 134](#8-references)] document, the Bill term is used in the
context of this document and the API to reuse the terms of TMF 678 API.

## 4.3. Approach

As presented in Figure 2 both Cantata and Sonata API frameworks consist of three
...
[[MEF134](#8-references)] in section 7. This picture aims to help understand the
...
`https://{serverBase}/mefApi/cantata/customerBillManagement/v3/`

**Base URL for Sonata**:

`https://{serverBase}/mefApi/sonata/customerBillManagement/v3/`

The following API endpoints are implemented by the Seller and allow the Buyer to
retrieve Customer Biils and register for Notifications. The endpoints and
...
**[O1]** The Seller implementation **MAY** support API endpoints listed in
Table 3. [MEF134 R2]
...
`https://{serverBase}/mefApi/cantata/customerBillNotification/v3/`

**Base URL for Sonata**:

`https://{serverBase}/mefApi/sonata/customerBillNotification/v3/`

The following API endpoints are implemented by the Buyer and allow the Seller to
send Notifications. The endpoints and corresponding data model are defined in:
...
and flows. It starts with Figure 6 and Table 6, presenting a list and short
description of all business use cases then presents the variants of end-to-end
interaction flows, and the following subchapters describe the API usage flow and
examples for each of the use cases.

Figure 6 presents an example of an end-to-end flow:

![End-to-End API Flows](media/useCaseBill.png)

**Figure 6. End-to-End API Flows**
...
- (8) The Buyer pays the bill and `CustomerBillItem` changes state to `settled`.
...
- (12,13) The Buyer requests detailed information about the `CustomerBill` based
  on `id`.
...
section 7 of MEF 134 [[MEF 134](#8-references)].
...
**[R6]** The Seller **MUST** support all of `CustomerBillEventType`: [MEF134 R1]

- `customerBillCreateEvent`
- `customerBillStateChangeEvent`

**[R7]** The Buyer's request **MUST** provide the `callback` attribute. [MEF134
R1]
...
to it, every attribute defined in the Event model (from the notification API)
can be used in the `query`. However, this standard requires only `eventType`
...
**[R9]** `eventType` is the only attribute that the Seller **MUST** support in
the query.
...
- `https://buyer.mef.com/listenerEndpoint/mefApi/sonata/customerBillManagement/v3/listener/customerBillCreateEvent`
...
**[R10]** The event sent by the Seller **MUST** contain following attributes:
[MEF134 R3]
...
- `event`:
  - `id`
  - `state` (in case of `CustomerBillStateChangeEvent`)

**[R11]** The `customerBillCreateEvent` **MUST** be sent when a `CustomerBill`
is created. [MEF134 R4]

**[R12]** The Seller **MUST** send Customer Bill Notifications to a Buyer who
has registered for the Customer Bill Notification Type.

**[R13]** The Seller **MUST NOT** send Customer Bill Notifications for
...
Figure 7 shows all entities involved in the Notification use cases.

![Billing Notification Data Model](media/useCase2NotificationModel.png)

**Figure 7. Use Case 2. Billing Notification Data Model**
...
The Buyer can get detailed information about the Bill from the Seller by using a
`GET /customerBill` operation.

**[R14]** The Buyer request **MUST** contain zero or more of the following query
parameters: [MEF134 R5]
...
**[R15]** The Seller response **MUST** include the following for each entry in
the list: [MEF134 R6]
...
**[R16]** If no `CustomerBill` matches the criteria provided by the Buyer, the
...
```
https://serverRoot/mefApi/sonata/customerBillManagement/v3/customerBill?state=generated&limit=10&offset=0
```
...
The Buyer can get detailed information about the Bill from the Seller by using a
`GET /customerBill/{{id}}` operation.

The Seller receives this request and returns a response.

**[R17]** The Seller response **MUST** include all of the attributes for the
...
1. This example describes the case when Bill is in a `generated` state. It means
   that the Buyer needs to pay it.
...
Figure 8 presents the CustomerBill state machine:

![CustomerBill State Machine](media/CustomerBillStates.png)

**Figure 8. CustomerBill State Machine**
...
1. A `CustomerBill` has been `generated` based on the completion of a
   ...
   Buyer.
2. In the `paymentDue` state, the Seller has completed their dispute process and
...
**[R18]** A CustomerBill API implementation **MUST** support the states and
transitions shown in Figure 8. [MEF134 R19]

## 6.5. Use Case 5: Retrieve Invoice Line Item by Identifier

The Buyer can get detailed information about the Bill from the Seller by using a
`GET /customerBillItem/{{id}}` operation.

The Seller receives this request and returns a response.

**[R19]** The Seller response **MUST** include the attributes for the returned
...
**[R20]** If the bill item relates to an instance of a Product then the
`product` reference **MUST** be provided. [MEF134 R10]

**[R21]** If the bill item relates to a Product Order Item then the
...
Figure 9 presents the `CustomerBillItem` state machine:

![CustomerBillItem State Machine](media/CustomerBillItemStates.png)

**Figure 9. CustomerBillItem State Machine**
...
4. In the `paymentDue` state, the Seller has completed their dispute process and
   they are expecting payment from the Buyer.
...
6. A Seller has agreed with the Buyer that disputed charges are in error and has
   `withdrawn` the CustomerBill.
...
| `disputeBeing- Investigated` | DISPUTE_BEING- \_INVESTIGATED | A Buyer has disputed charges included in a `CustomerBill` and those charges are in the Sellers Dispute Process. Disputes cannot exit the dispute process until the Dispute is resolved. |
...
**[R22]** A CustomerBill API implementation **MUST** support the states and
transitions shown in Figure 9. [MEF134 R20]

The interaction between `CustomerBillItem` and `CustomerBill` states is shown in
Figure 10:

![CustomerBillItem State Machine](media/CustomerBillCustomerBillItemStatesInteraction.png)

**Figure 10. Interaction between CustomerBillItem and CustomerBill States
Machine**

**[R23]** A CustomerBill API implementation **MUST** support logic shown in
Figure 10. [MEF134 R21]

## 6.6. Use Case 6: Retrieve Printable Invoice

**[R24]** If a printable document is agreed upon by the Buyer and Seller, then
...
**[R25]** The Buyer **MUST** use the `billDocument` provided by the Seller to
retrieve the Printable CustomerBill. [MEF134 R18]

**[R26]** The printable `CustomerBill` **MUST** be in a PDF format. [MEF134 R15]

**[R27]** The printable version of `CustomerBill` **MUST** contain all
...
**Figure 11. Data model types to represent an erroneous response**
...
Figure 12 presents the whole Billing Management data model the data types,
requirements related to them and mapping to MEF 134 specifications are discussed
later in this section.

![Billing Management Data Model](media/billingManagement.api.png)

**Figure 12. Billing Management Data Model**
...
<table id="T_CustomerBill" style="width:100%">
    ...
    <tbody>
        ...
        <td>category</td>
            <td><a href="#T_CustomerBillCategory">Customer-</br>BillCategory</a></td>
            <td>M</td>
            <td>The type of Bill. One of the following:-</br>- normal-</br>- duplicate-</br>- trial
</td>
        ...
        <td>state</td>
            <td><a href="#T_CustomerBillStateType">CustomerBill-</br>StateType</a></td>
            <td>M</td>
...
Product Order Item Reference Identifier </td>
        ...
        <td>state</td>
            <td><a href="#T_CustomerBillItemStateType">CustomerBill-</br>ItemStateType</a></td>
            <td>M</td>
...
<table id="T_CustomerBillItemFee" style="width:100%">
    ...
    <tbody>
        <tr>
        <td>category</td>
            <td><a href="#T_CustomerBillItemFeeCategory">CustomerBill-</br>ItemFeeCategory</a></td>
            <td>O</td>
...
**Description:** An identifier for the Billing Account that is unique within the
Seller
...
#### 7.2.2.5. Type FieldedAddressRepresentation
...
#### 7.2.2.10. Type RelatedContactInformation
...
<table id="T_EventSubscriptionInput" style="width:100%">
    ...
    <tbody>
        ...
        <td>callback</td>
            <td>string</td>
            <td>M</td>
            <td>This callback value must be set to *host* property from Buyer Notification API (billingNotification.api.yaml). This property is appended with the base path and notification resource path specified in that API to construct an URL to which notification is sent. E.g. for &quot;callback&quot;: &quot;http://buyer.com/listenerEndpoint&quot;, the state change event notification will be sent to: &#x60;http://buyer.com/listenerEndpoint/-</br>mefApi/sonata/customerBillManagement/-</br>v3/listener/troubleTicketStatusChangeEvent&#x60;</td>
            <td>Return Address Information</td>
        </tr><tr>
        <td>query</td>
            <td>string</td>
            <td>O</td>
            <td>This attribute is used to define to which type of events to register to. Example: &quot;query&quot;:&quot;eventType &#x3D; customerBillCreateEvent&quot;. To subscribe for more than one event type, put the values separated by comma: &#x60;eventType&#x3D;customerBillCreateEvent,-</br>customerBillStateChangeEvent&#x60;. The possible values are enumerated by the &#x27;CustomerBillEventType&#x27; in billingNotification.api.yaml. An empty query is treated as specifying no filters - ending in  subscription for all event types.</td>
            <td>Notification Type</td>
        </tr>
    </tbody>
</table>
...
Figure 13 presents the Billing Notification data model the data types,
requirements related to them and mapping to MEF 134 specifications are discussed
later in this section.

![Billing Notification Data Model](media/useCase2NotificationModel.png)
**Figure 13. Billing Notification Data Model**
...
### 7.3.2. Type CustomerBillCreateEvent
...
<table id="T_CustomerBillCreateEvent" style="width:100%">
    <thead style="font-weight:bold">
        <tr>
            <td>Name</td>
            <td style="width:15%">Type</td>
    ...
    <tbody>
        ...
        <td>event</td>
            <td><a href="#T_CustomerBillCreateEventPayload">CustomerBill-</br>CreateEventPayload</a></td>
            <td>M</td>
...
### 7.3.3. Type CustomerBillCreateEventPayload

**Description:** The identifier of the Bill being subject of this event.

<table id="T_CustomerBillCreateEventPayload" style="width:100%">
    <thead style="font-weight:bold">
        <tr>
            <td>Name</td>
            <td style="width:15%">Type</td>
...
### 7.3.4. Type CustomerBillStateChangeEvent
...
<table id="T_CustomerBillStateChangeEvent" style="width:100%">
    <thead style="font-weight:bold">
        <tr>
            <td>Name</td>
            <td style="width:15%">Type</td>
    ...
    <tbody>
        ...
        <td>event</td>
            <td><a href="#T_CustomerBillStateChangeEventPayload">CustomerBill-</br>StateChangeEventPayload</a></td>
            <td>M</td>
            <td>A reference to the object that is source of the notification.
</td>
            <td>Invoice Identifier</td>
...
### 7.3.5. Type CustomerBillStateChangeEventPayload

**Description:** The identifier of the Bill being subject of this event.

<table id="T_CustomerBillStateChangeEventPayload" style="width:100%">
    <thead style="font-weight:bold">
        <tr>
            <td>Name</td>
            <td style="width:15%">Type</td>
...
- [MEF 134](https://www.mef.net/wp-content/uploads/MEF-134.pdf), Billing and
  Invoice Business Requirements and Use Cases, February 2023
...
- [TMF678](https://www.tmforum.org/resources/specification/tmf678-customer-bill-management-api-user-guide-v4-0-0/),
  TMF678 Customer Bill Management API User Guide v4.0.1 Doc Checlist:
