...
- [5. API Description](#5-api-description)
  ...
  - [5.2. Resource/endpoint Description](#52-resourceendpoint-description)
    ...
    - [5.2.1. Buyer Side Endpoints](#521-buyer-side-endpoints)
...
- [7. API Details](#7-api-details)
  ...
  - [7.3. Notification API Data model](#73-notification-api-data-model)
    ...
    - [7.3.4. `enum` GeographicAddressValidationEventType](#734-enum-geographicaddressvalidationeventtype)
    - [7.3.5. `enum` GeographicAddressValidationStateType](#735-enum-geographicaddressvalidationstatetype)
...
- [`productApi/serviceability/address/geographicAddressManagement.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Sonata-SDK/refs/tags/irene/productApi/serviceability/address/geographicAddressManagement.api.yaml)

- [`productApi/serviceability/address/geographicAddressNotification.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Sonata-SDK/refs/tags/irene/productApi/serviceability/address/geographicAddressNotification.api.yaml)

<https://github.com/MEF-GIT/MEF-LSO-Cantata-SDK>


- [`productApi/serviceability/address/geographicAddressManagement.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Cantata-SDK/refs/tags/irene/productApi/serviceability/address/geographicAddressManagement.api.yaml)

- [`productApi/serviceability/address/geographicAddressNotification.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Cantata-SDK/refs/tags/irene/productApi/serviceability/address/geographicAddressNotification.api.yaml)
<!-- TODO CANTATA and commit ID -->
...
<table>
  ...
  <tr>
    ...
    <td>This document; adapted from <a href="#8-references">[MEF 150]</a></td>
  </tr>  
  <tr>
    ...
    <td><a href="#8-references">[MEF 150]</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>This document; adapted from <a href="#8-references">[MEF 150]</td>
  </tr>
  <tr>
...
Requirements and Use Cases_ [[MEF150](#8-references)] which introduced the
...
of `GeographicAddress`, according to before-mentioned MEF Standard.
...
consistent with [MEF 150](#8-references). The underlined font designates
required use cases.
...
| API endpoint                            | Use Case Name                                        | MEF 150 Use case Mapping                                                                             |
| --------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
...
this request is associated [MEF150 R62]
...
codes. [MEF150 R8], [MEF150 R11], [MEF150 R13]
...
Geographic Address. [MEF150 R2], [MEF150 R6], [MEF150 O3]

**[O1]** `streetName` **MAY** contain the `streetType` or `streetType` **MAY**
be used as a stand-alone field. [MEF150 O1]
...
the end of the range. [MEF150 O2]
...
`FormattedAddressRepresentation`: [MEF150 R11]

- `formattedAddress`

**[R11]** The Buyer and Seller **MUST** agree if they also require the
`language` attribute for `FormattedAddressRepresentation`. [MEF150 R2]
...
`LabelRepresentation`: [MEF150 R12]

- `administrativeAuthority`
- `label`

**[R13]** The Buyer and Seller **MUST** agree if they also require the
`language` attribute for `LabelRepresentation`. [MEF150 R2]
...
`GeographicPointRepresentation`: [MEF150 R14]
...
between Buyer and Seller during the onboarding process. [MEF150 R16]

**[R16]** The Buyer and Seller **MUST** agree to the level of accuracy of the
`latitude`, `longitude` and `elevation`. [MEF150 R15]
...
`elevation` attribute for `GeographicPointRepresentation`. [MEF150 R2]
...
**[R20]** The Buyer **MUST** be able to initiate Use Case 1 and 3. [MEF150 R17],
[MEF150 R39]

**[R21]** The Seller **MUST** be able to provide either an Immediate Response or
a Deferred Response for Use Case 1. [MEF150 R19]

**[R22]** The Seller **MUST** associate an `id` to each unique Geographic
Address of which they are aware. [MEF150 R18]

**[R23]** If a Seller supports Deferred Response for the Validate Geographic
Address request, the Seller **MUST** support Use Cases 2, 6, and 7. [MEF150
R120]
...
`GeographicAddress.id` and it's Representations. [MEF150 D2]
...
`GeographicAddressValidation_Create`: [MEF150 R22], [MEF150 R23]
...
asked for an immediate response (`instantSyncValidation=true`). [MEF150 R25]

**[O3]** The Seller **MAY** provide an immediate response regardless of the
`instantSyncValidation` the Buyer has asked for. [MEF150 O6]
...
attributes received in the Buyer's request. [MEF150 R29]

**[R32]** Following attributes **MUST** be provided when specifying the
response: [MEF150 R27], [MEF150 R28]
...
`GeographicAddressValidation.id`. [MEF150 R35]
...
successful response with an empty list of `bestMatchGeographicAddress` and
`alternateGeographicAddress`.

**[R37]** If a `GeographicAddress` is identified and returned as a
`bestMatchGeographicAddress` it **MUST NOT** be returned in
`alternateGeographicAddress` list. [MEF150 R32]
...
that the Geographic Address is within their Area of Validation. [MEF150 R33]
...
attributes. [MEF150 R37]

This may result in empty `bestMatchGeographicAddress` and/or
`alternateGeographicAddress` attributes being returned if the Seller has not
completed the validation process yet.
...
Retrieve Geographic Address by Identifier request. [MEF150 R41]
...
**[R46]** The Buyer request **MUST** contain the following attributes: [MEF150
R55]
...
used in the `query`. However, this MEF standard requires only `eventType`
...
have registered for them. [MEF150 R57]

**[R52]** The Seller **MUST NOT** send Notifications for `eventTypes` to Buyers
who have not registered for them. [MEF150 R58]

**[R53]** The Seller **MUST** send a notification to all of the targets
specified by the Buyer every time there is a change in state. [MEF150 R60]
...
requirements related to them, and mapping to MEF 150 specification are discussed
...
<table id="T_GeographicAddressValidationEvent" style="width:100%">
    ...
    <tbody>
        <tr>
        <td>eventType</td>
            <td><a href="#T_GeographicAddressValidationEventType">GeographicAddressValidationEventType</a></td>
            <td>M</td>
...
### 7.3.4. `enum` GeographicAddressValidationEventType

**Description:** Indicates the type of Geographic Address Validation event.

<table id="T_GeographicAddressValidationEventType">
    <thead style="font-weight:bold;">
        <tr>
            <td>Value</td>
            <td>MEF 150</td>
    ...
    <tbody>
        <tr>
            <td>geographicAddressValidationStateChangeEvent</td>
            <td>GEOGRAPHIC_ADDRESS_VALIDATION_STATE_CHANGE_EVENT</td>
        </tr>
    </tbody>
</table>

### 7.3.5. `enum` GeographicAddressValidationStateType
