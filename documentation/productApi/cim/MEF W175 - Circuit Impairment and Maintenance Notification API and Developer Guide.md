...
[[MEF 113](#8-references)] and MEF 173 _CIM Notification Service Business
Requirements and Use Cases_ [[MEF 173](#8-references)]. This standard consists
of this document and complementary API definitions for CIM and CIM Notification.
...
- [`productApi/cim/circuitImpairmentAndMaintenance.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Sonata-SDK/refs/tags/janis/productApi/cim/circuitImpairmentAndMaintenance.api.yaml)
- [`productApi/cim/circuitImpairmentAndMaintenanceNotification.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Sonata-SDK/refs/tags/janis/productApi/cim/circuitImpairmentAndMaintenanceNotification.api.yaml)
...
- [`productApi/cim/circuitImpairmentAndMaintenance.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Cantata-SDK/refs/tags/janis/productApi/cim/circuitImpairmentAndMaintenance.api.yaml)
- [`productApi/cim/circuitImpairmentAndMaintenanceNotification.api.yaml`](https://raw.githubusercontent.com/MEF-GIT/MEF-LSO-Cantata-SDK/refs/tags/janis/productApi/cim/circuitImpairmentAndMaintenanceNotification.api.yaml)
...
- MEF 150 [[MEF 150](#8-references)]
- MEF 173 [[MEF 173](#8-references)]
...
<tr>
  ...
  <td><a href="#8-references">[MEF 173]</td>
</tr>
<tr>
...
by `id` and register for receiveing Notifications, and the Seller to send
...
Ticketing Requirements and Use Cases_ [[MEF 113](#8-references)] and MEF 173
_CIM Notification Service Business Requirements and Use Cases_
[[MEF 173](#8-references)]. MEF 113 defines use cases that cover Trouble Ticket,
Incident, Appointment, and WorkOrder. MEF W175 defines additional requirements
on top of those defined by MEF 113 for Incident. This document defines the CIM
API as a separate, standalone single-purpose API and thus MEF W175 requirements
may take precedence above ones defined in MEF 113. This document is structured
as follows:
...
in MEF 113 [[MEF 113](#8-references)] and MEF 173 _CIM Notification Service
Business Requirements and Use Cases_ [[MEF 173](#8-references)]. This API
...
- Circuit Impairment and Maintenance:
  - Allows the Buyer to list CIM Incidents, get CIM Incidents details and
    register for receiveing Notifications, and the Seller to send Notifications
...
Figure 4 presents a high-level use case diagram as specified and numbered in MEF
173 [[MEF 173](#8-references)] in section 7.2. Use cases are described
...
receiveing Notifications. The endpoints and corresponding data model are defined
in:

`productApi\cim\circuitImpairmentAndMaintenance.api.yaml`.

| API endpoint              | Description                                                                                                                                                      | MEF 113 / MEF 173 Use Case mapping                      |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /listener/CimIncidentCreateEvent`               | A request initiated by the Seller to notify the Buyer on `CimIncident` creation                | MEF 113 UC 18: Send Notification</br>MEF 173 UC 1: Failure Notification</br>MEF 173 UC 2: Service Impairment Notification</br>MEF 173 UC 3: Maintenance Notification |
| `POST /listener/CimIncidentAttributeValueChangeEvent` | A request initiated by the Seller to notify the Buyer on `CimIncident` attribute value change. | MEF 113 UC 18: Send Notification</br>MEF 173 UC 1: Failure Notification</br>MEF 173 UC 2: Service Impairment Notification</br>MEF 173 UC 3: Maintenance Notification |
| `POST /listener/CimIncidentStatusChangeEvent`         | A request initiated by the Seller to notify the Buyer on `CimIncident.status` change.          | MEF 113 UC 18: Send Notification</br>MEF 173 UC 1: Failure Notification</br>MEF 173 UC 2: Service Impairment Notification</br>MEF 173 UC 3: Maintenance Notification |
...
Retrieve CIM Incident List poll request. [MEF173 D4]
...
Incident List poll request. [MEF173 D6]
...
`status` is `closed`. [MEF113 R133], [MEF173 R18]
...
Figure 6 presents the Incident state machine:

![Incident state Machine](media/cimIncidentStates.png)

**Figure 6. Incident State Machine**

**[R13]** The Seller **MUST** support all CIM Incident statuses and their
associated transitions as described in Figure 6 and Table 6. [MEF113 R167]

![Use Case 16: CIM Incident Model](media/cimIncidentModelNoErrors.png)

**Figure 7. Use Case 16: CIM Incident Model**
...
**[R14]** The Seller **MUST** support all types of events: [MEF173 R21]

- `cimIncidentAttributeValueChangeEvent`
- `cimIncidentCreateEvent`
- `cimIncidentStatusChangeEvent`

**[D6]** The Buyer **SHOULD** register for all types of events. [MEF173 D3],
[MEF173 D7]
...
fulfills the [MEF113 R137], [MEF113 R138], [MEF113 R139], [MEF173 R13], [MEF173
R14] requirements.
...
`CimIncident` instance if they are set. [MEF173 R17]

The Figure 8 shows entities involved in the Notification use cases. Every Event
...
skipped as they are same as on Figure 7.

![Notification Data Model](media/cimIncidentNotificationModel.png)

**Figure 8. Notification Data Model**
...
with sufficient details about the nature of the Failure Situation. [MEF173 R3]
...
attribute value to `degraded` or `intermittent`. [MEF173 R6]
...
time of the Service Impairment. [MEF173 D2]
...
Figure 10 presents the whole CIM data model followed by Figure 11 with all
...
**Figure 10. CIM Management Data Model**

![CIM Data Model](media/cimIncidentModelEnums.png)

**Figure 11. CIM Management Data Model - Enums**
...
<table id="T_CimIncident" style="width:100%">
    ...
    <tbody>
        ...
        <td>@type</td>
            <td>string</td>
            <td>O</td>
            <td>Used to unambiguously designate the class type when sublassing</td>
            <td></td>
        </tr><tr>
        <td>attachment</td>
        ...
        <td>priority</td>
            <td><a href="#T_CimIncidentPriorityType">CimIncidentPriorityType</a></td>
            <td>O</td>
...
Incident Technical Contact (&#x27;role&#x3D;incidentTechnicalContact&#x27;)</td>
        ...
        <td>relatedIssue</td>
            ...
            <td>O</td>
            <td>A list of Related Issue relationships. Represents relationships to other Trouble Tickets, Incidents, and CIM Incidents.</td>
            <td></td>
        </tr><tr>
        <td>severity</td>
            <td><a href="#T_CimIncidentSeverityType">CimIncidentSeverityType</a></td>
            <td>O</td>
...
specifications or is not opera-tional on an intermittent basis. This includes
impair-ments due to traffic violating traffic profile thresholds (for example,
...
- failure - Any Situation where the Product is non-operational, such as down,
  routing fail-ures or severe packet drops.

- plannedMaintenance -Any scheduled maintenance that may impact a Product for
  the Buyer.

- policing - Any Situation due to traffic violating traffic profiles as-sociated
...
<table id="T_EventSubscriptionInput" style="width:100%">
    ...
    <tbody>
        ...
        <td>callback</td>
            <td>string</td>
            <td>M</td>
            <td>This callback value must be set to *host* property from Buyer Notification API (circuitImpairmentAndMaintenance.api.yaml). This property is appended with the base path and notification resource path specified in that API to construct an URL to which notification is sent. E.g. for &quot;callback&quot;: &quot;http://buyer.mef.com/listenerEndpoint&quot;, the status change event notification will be sent to: &#x60;http://buyer.mef.com/listenerEndpoint/mefApi/sonata/cimIncidentNotification/v1/listener/cimIncidentAttributeValueChangeEvent&#x60;</td>
            <td>Notification Target Information</td>
        </tr><tr>
        <td>query</td>
            <td>string</td>
            <td>O</td>
            <td>This attribute is used to define to which type of events to register to. Example: &quot;query&quot;:&quot;eventType &#x3D; cimIncidentAttributeValueChangeEvent&quot;. To subscribe for more than one event type, put the values separated by comma: &#x60;eventType&#x3D;cimIncidentAttributeValueChangeEvent,cimIncidentCreateEvent&#x60;. An empty query is treated as specifying no filters - ending in subscription for all event types.</td>
...
Figure 12 presents the CIM Notification data model.

![CIM Notification Data Model](media/cimIncidentNotificationModel.png)

**Figure 12. CIM Notification Data Model**
...
- [MEF 173](https://www.mef.net/wp-content/uploads/MEF-173.pdf) CIM Notification
  Service Business Requirements and Use Cases, May 2025
