...
<xsl:stylesheet version="2.0"
  ...
  <!--
     XSLTJSON v1.0.93.
     CUSTOM FOR API DESIGNER
     * string and boolean type only
     * skip root
  ...
  <xsl:function name="json:create-node" as="node()">
    ...
    <xsl:choose>
      <xsl:when test="$in-array">
        <json:array-value>
          <xsl:choose>
            <xsl:when  test="exists($node/@datatype)">
              <xsl:text/><xsl:value-of select="$node"/><xsl:text/>
            </xsl:when>
            <xsl:otherwise>
              <json:value>
  ...
  <xsl:function name="json:create-children">
    ...
    <xsl:choose>
      <xsl:when test="exists($node/child::text()) and count($node/child::node()) eq 1">
        <xsl:choose>
          <xsl:when test="(count($node/namespace::*) gt 0 and $use-namespaces) or count($node/@*[(not(../@json:force-array) and not(.|../@datatype)) or count(.|../@json:force-array)=2]) gt 0">
            <json:object>
